...
const AdmitCardDetail = () => {
  ...
  const get_admit_card_detail = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.post(API_ENDPOINTS.CBT.GET_ADMIT_CARD, {
  ...
  const handleAtemptTest = async () => {
    try {
      const res_attempt = await axiosInstance.post(
        "/v2_data_model/get_cbt_check_date",
        { test_id: test_id }
      );
      ...
      console.error("Error while attempting test:", error);
      toast.error("Failed to attempt test. Please try again later.");
    }
  };
                ...
                <div className="col-4 col-md-3 col-lg-2">
                  <div className="profileimg">
                    <img
                      ...
                      loading="lazy" alt={admitCardDetail?.user_admit_card?.candidateName || ""}
                    />
                  </div>
                </div>
